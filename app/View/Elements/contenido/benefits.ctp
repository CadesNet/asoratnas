			      ...
			      <?php foreach ($carousel as $carousels) { ?>
			        ...
			        <li>
			        	<?php if($this->Session->read('Auth.User.id')){ ?>
							...
							<?php echo $this->Form->postLink(__("<i class='icon-remove'></i>"), array('controller'=>'Carousels','action' => 'delete', $carousels['Carousel']['id'],'1','1','1','beneficios'),array('class' => 'ok btn btn-info ','escape' => false), __('¿Está seguro de que desea eliminar esta imagen?')); 
						}?>
								...
								<div class="row-fluid">
									<div class="span12">
										<h4 class = "titlebeneficios"><?php echo  ((strlen(h($benefit['Benefit']['title']))>47) ? substr(h($benefit['Benefit']['title']), 0, 47)."..." : substr(h($benefit['Benefit']['title']), 0, 47));
										 ...
										 echo $this->Form->postLink(__("<i class='icon-remove'></i>"), array('Controller'=>'Benefits','action' => 'delete', $benefit['Benefit']['id'],'mm'),array('class' => 'ok btn btn-info ','escape' => false), __('¿Está seguro de que desea eliminar el beneficio %s?',$benefit['Benefit']['title'])); }?></h4>
									</div>
								</div>
								<div class="row-fluid">
										<div class="span12">
